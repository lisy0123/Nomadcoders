...
- **나쁜 주석**
  ...
  7. **위치를 표시하는 주석**
     ...
     너무 자주 사용하지 않는다면 눈에 띄며 주의를 환기하는 역할을 하기 때문에 반드시 필요할 때만, 아주 드물게 사용하는 편이 좋다.

  8. **닫는 괄호에 다는 주석**

     중첩이 심하고 장환한 함수라면 의미가 있을 수도 있지만 작고 캡슐화된 함수에는 필요하지 않다.
     닫는 괄호에 주석을 달아야겠다는 생각이 든다면 함수를 줄이려고 시도해야 한다.
  
  9. **공로를 돌리거나 저자를 표시하는 주석**
  
     소스 코드 관리 시스템에 저장하면 되기 때문에 코드에 주석으로 표시하는 것은 불필요한 정보이다.
